...
export default function cargoPage(props) {
  ...
  function excluir(cargo) {
    if (window.confirm(`Deseja realmente excluir ${cargo.nome}`)) {
      axios.delete(`http://localhost:3000/cargos/${cargo._id}`).then(() => {
              ...
              return (
                <tr>
                  ...
                  <td className="text-center">
                    ...
                    <Button
                      className="me-2"
                      href={`/cargos/edit?_id=${cargo._id}`}
                    >
                      <FaPen />
                    </Button>
                    <Button variant="danger" onClick={() => excluir(cargo)}>
                      <FaTrash />
                    </Button>
                  </td>
                </tr>
